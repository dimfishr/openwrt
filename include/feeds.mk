...
EXCLUDE_FEEDS:=nss_packages sqm_scripts_nss awgopenwrt qosmio

# 1: destination file
define FeedSourcesAppendOPKG
( \
  echo 'src/gz %d_core https://openwrt.admincomps.ru/24.10-nss/$(shell date +"%Y%m%d")/packages'; \
  $(strip $(if $(CONFIG_PER_FEED_REPO), \
...
( \
  echo 'https://openwrt.admincomps.ru/25.12-nss/$(VERSION_NUMBER)/packages/packages.adb'; \
  $(strip $(if $(CONFIG_PER_FEED_REPO), \
